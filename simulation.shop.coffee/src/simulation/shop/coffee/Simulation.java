...
public class Simulation {
	...
	public static Queue<Customer> customersInQueue = new LinkedList<Customer>();
	public static Queue<Customer> customerOrderList = new LinkedList<Customer>();
	public static Map<Customer, Boolean> customerOrdersStatus = new HashMap<Customer, Boolean>(); 

	public static Queue<Customer> customersInShop;
	...
	public static List<SimulationEvent> runSimulation(int numCustomers, int numCooks, int numTables,
		...
		customersInShop = new PriorityQueue<Customer>(numCustomers,
				Comparator.comparing(Customer::getCusPriority));
		...
		if (!randomOrders) {
			...
			for (int i = 0; i < customers.length; i++) {
				Customer cp = new Customer("Customer " + (i + 1), order, new Random().nextInt(5) + 1);
				customers[i] = new Thread(cp);
				customersInShop.offer(cp);
			}
		} else {
			for (int i = 0; i < customers.length; i++) {
				...
				Customer c = new Customer("Customer " + (i + 1), order, new Random().nextInt(5) + 1);
				customers[i] = new Thread(c);
				customersInShop.offer(c);
			}
		...
		try {
			...
			SimulationEvent.LOGGER.error(e.getMessage(), e);
